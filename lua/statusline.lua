...
vim.cmd('highlight GalaxyRightLspErrorActive        guibg=#504945 guifg=#fb4632')
vim.cmd('highlight GalaxyRightLspHintActive         guibg=#504945 guifg=#27b31a')
vim.cmd('highlight GalaxyRightLspInformationActive  guibg=#504945 guifg=#127fff')
vim.cmd('highlight GalaxyRightLspWarningActive      guibg=#504945 guifg=#fe811b')
    ...
    LeftViMode = {
      ...
      provider = function()
        if vi_mode_mapping[vim.fn.mode()] == nil then
          ...
          return ' ' .. string.upper(vi_mode_mapping[vim.fn.mode()][1])
        end
      end,
      separator = ' ',
    ...
    LeftGitDiffAdd = {
      ...
      provider = function()
        if require('galaxyline.provider_vcs').diff_add() then
          vim.cmd('highlight link GalaxyLeftGitDiffAdd GalaxyMapperCommon6')
    ...
    LeftGitDiffModified= {
      ...
      provider = function()
        if require('galaxyline.provider_vcs').diff_modified() then
          vim.cmd('highlight link GalaxyLeftGitDiffModified GalaxyMapperCommon6')
    ...
    LeftGitDiffRemove = {
      ...
      provider = function()
        if require('galaxyline.provider_vcs').diff_remove() then
          vim.cmd('highlight link GalaxyLeftGitDiffRemove GalaxyMapperCommon6')
    ...
    RightLspError = {
      provider = function()
        ...
        if vim.lsp.diagnostic.get_count(0, 'Error') == 0 then
          vim.cmd('highlight link GalaxyRightLspError GalaxyMapperCommon4')
        else
    ...
    RightLspWarning = {
      provider = function()
        ...
        if vim.lsp.diagnostic.get_count(0, 'Warning') == 0 then
          vim.cmd('highlight link GalaxyRightLspWarning GalaxyMapperCommon4')
        else
    ...
    RightLspHint = {
      provider = function()
        ...
        if vim.lsp.diagnostic.get_count(0, 'Hint') == 0 then
          vim.cmd('highlight link GalaxyRightLspHint GalaxyMapperCommon4')
        else
